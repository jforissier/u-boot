...
static void run_threads(const char *msg, const char *type)
{
#if CONFIG_IS_ENABLED(UTHREAD)
	if (!nthr)
		return;
	printf("%s %d %sUSB bus(es)... ", msg, nthr, type);
...
int usb_init(void)
{
	...
	if (CONFIG_IS_ENABLED(UTHREAD))
		run_threads("Scanning", "companion ");
